...
describe PhocoderController do
  ...
  describe "POST 'phocoder_update'" do
    ...
    it "should update an input" do
      post 'phocoder_update', {:class=>"ImageUpload",:id=>1,:input=>{:id=>1,:file_size=>2,:width=>2,:height=>2},:format=>"json" }
      response.should be_success
      @image_upload.reload
    ...
    it "should update an output" do
      post 'phocoder_update', {:class=>"ImageUpload",:id=>1,:output=>{:id=>1,:file_size=>2,:width=>2,:height=>2},:format=>"json" }
      response.should be_success
      @image_upload.reload
      @image_upload.file_size.should == 2
    end
